...
const BuscadorNormalContabilidad = ({
	...
	const HandlerOnChange = (event) => {
		setClienteContabilidadBuscado(Busqueda.current.value);
	};
		...
		if (event.key === "Enter") {
			if (event.target.value != "" || event.target.value != null) {
				...
				if (event.target.value.length <= 4) {
					...
					actions.getBusquedaContabilidad(event.target.value);
				}
			}
		}
	};
		...
		<div className='cell small-10'>
			<div className='grid-x grid-margin-x'>
				{store.usuarioActual != null ? (
					...
					store.usuarioActual.tipo === "Cobranza" ? null : (
						<>
							<input
								...
								placeholder='Ingresa tu Busqueda (Rut Empresa sin puntos, razon social, correo, Nombre o Rut Representante)'
								name='Buscador'
								ref={Busqueda}
							/>
						</>
...
export default BuscadorNormalContabilidad;
